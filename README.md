...
Самый простой и рекомендуемый способ запустить все компоненты проекта на Windows — использовать `start.bat`.

1.  Убедитесь, что у вас установлен \`Go\` последней версии.
2.  Перейдите в корневую директорию проекта.
3.  Дважды кликните по \`start.bat\` или выполните в консоли:

    ```bash
    .\start.bat
    ```

    Этот скрипт автоматически настроит локальные переменные среды и запустит Оркестратор и Агент в параллельных окнах командной строки.

Или, если вы хотите запускать вручную (в разных терминалах):
