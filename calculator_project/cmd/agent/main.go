package main // Основной исполняемый пакет для сервиса Агента

import (
	"context" // Для работы с контекстом (например, для таймаутов подключения к Оркестратору)
	"fmt"
	"log" // Для логирования сообщений о работе Агента
	"os"  // Для чтения переменных среды (конфигурация Агента)
	"strconv"
	"time" // Для работы со временем (например, таймауты, задержки между запросами)

	// Импортируем сгенерированный gRPC код клиента
	// 'pb' - это алиас, чтобы не писать полное имя пакета каждый раз
	agent "calculator_project/internal/agent"
	pb "calculator_project/internal/orchestrator/grpc"

	"google.golang.org/grpc"                      // Основная библиотека gRPC для Go
	"google.golang.org/grpc/credentials/insecure" // Используем нешифрованное соединение для простоты разработки. В продакшене требуется TLS.
)
...
type Config struct {
	...
	// TODO: Добавить другие настройки Агента, если потребуются (например, количество одновременных задач).
}
...
func loadConfig() (*Config, error) {
	...
	// TODO: Добавить проверки обязательных параметров конфигурации, если они появятся.
...
func main() {
	...
	for { // Бесконечный цикл. Агент будет работать до принудительной остановки.
		...
		// Используем анонимную функцию для создания новой области видимости для контекста и defer cancel().
		// Это гарантирует, что cancel() будет вызван в конце КАЖДОЙ итерации цикла обработки задачи.
		// cmd/agent/main.go (Inside the for {} loop)
		func() {
			...
			log.Printf("Получена задача %s: Операция='%s', Аргументы='%s', '%s', Длительность=%dms",
				task.GetId(), task.GetOperation(), task.GetArg1(), task.GetArg2(), task.GetDurationMs()) // <--- ОБНОВЛЕНО: Добавлено логирование длительности

			// *** Добавляем задержку перед вычислением (Шаг 15.6) ***
			// Получаем длительность в миллисекундах (int64) из поля DurationMs protobuf задачи.
			durationMs := task.GetDurationMs()
			if durationMs > 0 {
				// Преобразуем int64 миллисекунд в тип time.Duration для использования с time.Sleep.
				duration := time.Duration(durationMs) * time.Millisecond
				log.Printf("Задача %s: Имитация выполнения операции, пауза %dms...", task.GetId(), durationMs)
				time.Sleep(duration) // <-- ВЫПОЛНЯЕМ ПАУЗУ!
				log.Printf("Задача %s: Пауза %dms завершена.", task.GetId(), durationMs)
			} else {
				log.Printf("Задача %s: Длительность 0ms, без паузы.", task.GetId())
			}

			// *** Логика вычисления задачи (Шаг 3 - уже реализована) ***
			// Этот код выполняется СРАЗУ после блока time.Sleep или сразу после логирования, если длительность 0.
			...
			// --- Обработка разных типов задач/операций ---
			...
			if task.GetOperation() == "number" {
				...
				if err1 != nil || err2 != nil {
					...
					// *** Вызываем вынесенную функцию agent.CalculateOperation для выполнения бинарной операции ***
			...
			if err != nil {
				...
				// TODO: В реальной системе нужно продумать механизм повторных попыток отправки результатов,
				...
				// TODO: Аналогично, нужна логика обработки неподтвержденных результатов.
			} else {
